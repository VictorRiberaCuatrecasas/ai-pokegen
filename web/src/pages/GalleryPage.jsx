...
export default function GalleryPage() {
  ...
  if (error) return <div className="text-red-500 dark:text-red-400">Failed to load: {String(error)}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((p) => (
            <div key={p.id} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <img src={`${API}${p.image_url}`} alt={p.name} className="w-full aspect-square object-contain" />
